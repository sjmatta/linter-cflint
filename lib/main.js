...
export default {
  config: {
    ...
    configFile: {
      type: 'string',
      default: '',
      description: 'cflint config file name (default .cflingrc)',
    },
  },
    ...
    return {
      ...
      async lint(textEditor) {
        ...
        async function doLint() {
          ...
          // if (filePath) {
          //   javaArgs.push("-stdin", filePath);
          // }
          console.log(javaPath, javaArgs);
          let jsonResult = await exec(javaPath, javaArgs, {
            stdin: fileText,
            stdio: 'pipe',
            stream: 'stdout',
            encoding: 'utf8',
          ...
          const messages = _(result.issues).map((issue) => {
            ...
            const message = issueLocation.message;
            return {
              filePath,

...
import { find, exec, execNode, generateRange } from 'atom-linter';
    ...
    return {
      ...
      async lint(textEditor) {
        ...
        async function doLint() {
          ...
          const cflintConfig = find(configPath, '.cflintrc');
          ...
          javaPath = `${packagePath}/node_modules/cflint/bin/cflint.js`;
/*

          let childProcess: ChildProcess = spawn(javaPath, javaArgs);
          console.log(`[${new Date()}] ${javaPath} ${javaArgs.join(" ")}`);
          const data = { stdout: [], stderr: [] }

          if (childProcess.pid) {
            //childProcess.stdin.write(textEditor.getText(), "utf-8");
...
*/
          ...
          //console.log(`[${new Date()}] ${javaPath} ${javaArgs.join(" ")}`);
          const jsonResult = await execNode(javaPath, javaArgs, {
